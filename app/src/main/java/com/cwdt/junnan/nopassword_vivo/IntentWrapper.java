...
public class IntentWrapper {
    ...
    public static List<IntentWrapper> getIntentWrapperList() {
        if (sIntentWrapperList == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                PowerManager pm = (PowerManager) NoPassApplication.getInstance().getSystemService(Context.POWER_SERVICE);
                boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(NoPassApplication.getInstance().getPackageName());
                if (!ignoringBatteryOptimizations) {
                    Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                    dozeIntent.setData(Uri.parse("package:" + NoPassApplication.getInstance().getPackageName()));
            ...
            xiaomiGodIntent.putExtra("package_name", NoPassApplication.getInstance().getPackageName());
            ...
            //三星 5.0/5.1 自启动应用程序管理
            Intent samsungLIntent = NoPassApplication.getInstance().getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
            ...
            meizuIntent.putExtra("packageName", NoPassApplication.getInstance().getPackageName());
    ...
    public static String getApplicationName() {
        if (sApplicationName == null) {
            ...
            ApplicationInfo ai;
            try {
                pm = NoPassApplication.getInstance().getPackageManager();
                ai = pm.getApplicationInfo(NoPassApplication.getInstance().getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName = NoPassApplication.getInstance().getPackageName();
            }
        }
    ...
    protected boolean doesActivityExists() {
        PackageManager pm = NoPassApplication.getInstance().getPackageManager();
